...
namespace RunAsHelper.ViewModel
{
    ...
    {
        private static readonly string RunAsHelperExe;

        private static readonly string[] ExistingApplications;


        static MainViewModel()
        {
            RunAsHelperExe = new FileInfo(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "RunAsHelper.exe")).FullName;
        ...
        private void StartWithWindowsCommand()
        {
            ...
            {
                ProcessStartInfo info = new ProcessStartInfo(RunAsHelperExe);
        ...
        public static void ModifyStartWithWindows(bool enabled)
        {
            ...
            if (enabled == true)
            {
                task = task ?? TaskService.Instance.AddTask("Auto Start RunAsHelper", QuickTriggerType.Logon, RunAsHelperExe);
        ...
        private static void StartApplication(ApplicationMenuItemViewModel applicationMenuItemViewModel)
        {
            ...
            if (applicationMenuItemViewModel.ApplicationViewModel.RunAsLocalAdmin && !hasAdministrativeRight)
            {
                ...
                var info = new ProcessStartInfo(RunAsHelperExe)
                {
                    Arguments = payload,
        ...
        private void CreateDesignTimeData()
        {
            ...
            {
                new ProfileViewModel {Domain = "MyDomain1", Username = "MyUsername1@MyDomain1"},
                new ProfileViewModel {Domain = "MyDomain1", Username = "MyUsername1@MyDomain1"},
                new ProfileViewModel {Domain = "MyDomain1", Username = "MyUsername1@MyDomain1"},
                new ProfileViewModel {Domain = "MyDomain1", Username = "MyUsername1@MyDomain1"}
            };
        ...
        private void AddProfileCommand()
        {
            ...
            var dialog = new CredentialsDialog(profileViewModel)
            {
                Owner = OptionsView.CurrentInstance,
                WindowStartupLocation = WindowStartupLocation.CenterOwner
            };
        ...
        private void EditProfileCommand()
        {
            ...
            var dialog = new CredentialsDialog(tempProfile)
            {
                Owner = OptionsView.CurrentInstance,
                WindowStartupLocation = WindowStartupLocation.CenterOwner
            };
